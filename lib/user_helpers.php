...
{
    if (is_logged_in() && isset($_SESSION["user"]["account"])) {
        return se($_SESSION["user"]["account"], "account_number", 0, false);
    }
}
function get_firstname()
{
    if (is_logged_in()) { //we need to check for login first because "user" key may not exist
        return se($_SESSION["user"], "first_name", false, false);
    }
    return false;
}
...
{
    if (is_logged_in()) { //we need to check for login first because "user" key may not exist
        return se($_SESSION["user"], "last_name", false, false);
    }
    return false;
}
...
{
    if (is_logged_in()) {
        return se($_SESSION["user"]["account"], "account_type", false, false);
    }
    return false;
}
